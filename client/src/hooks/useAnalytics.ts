...
import {
  ...
  profitOverTime, drawdownSeries, groupDuplicateBets,
...
export function useAnalytics() {
  ...
  return useMemo(() => {
    // Collapse repeat placements of the same bet before counting outcomes.
    const logicalBets = groupDuplicateBets(filteredBets);
    const kpis = computeKpis(filteredBets, logicalBets);
    ...
    const prevKpis = prevBets && prevBets.length
      ? computeKpis(prevBets, groupDuplicateBets(prevBets))
      : null;
